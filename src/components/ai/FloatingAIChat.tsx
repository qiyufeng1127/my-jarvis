...
请返回JSON格式，包含location字段（bathroom/workspace/kitchen/livingroom/bedroom/studio）。`;
    ...
    try {
      ...
      // 检测是否是任务创建/分解请求
      const isTaskCreation = /创建|添加|新建|安排|计划|做|完成|学习|工作|运动|分解|拆解/.test(message);
      const needsDecompose = /分解|拆解|详细安排|具体步骤/.test(message) || message.length > 20;
        ...
        if (needsDecompose && hasAI) {
          try {
            ...
            if (decomposeResult.success && decomposeResult.tasks && decomposeResult.tasks.length > 0) {
              ...
              responseContent += `我帮你把任务分解成了 ${decomposeResult.tasks.length} 个具体步骤：\n\n`;
              
              decomposeResult.tasks.forEach((task, index) => {
                const priorityEmoji = task.priority === 'high' ? '🔴' : task.priority === 'medium' ? '🟡' : '🟢';
                responseContent += `${index + 1}. ${priorityEmoji} **${task.title}**\n`;
                responseContent += `   ⏱️ ${task.duration} 分钟`;
                if (task.startTime) {
                  responseContent += ` | 🕐 ${task.startTime}`;
                }
                responseContent += `\n\n`;
              });
              ...
              responseContent += '💡 回复"确认"或"创建"，我就帮你添加到时间轴！';
              ...
              const aiMessage: Message = {
                ...
                pendingAction: {
                  type: 'create_tasks',
                  data: {
                    tasks: decomposeResult.tasks.map(t => ({
                      ...t,
                      description: message,
                    })),
                    goalMatches,
