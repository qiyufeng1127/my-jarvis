...
import { X, Plus, Search, Trash2, Check, ChevronRight, ShieldAlert, RotateCcw } from 'lucide-react';
...
import { createSubTasksFromTitles, getStepEmojiRecommendations, type SubTask } from '@/services/taskVerificationService';
...
export default function CompactTaskEditModal({ task, onClose, onSave, onDelete }: CompactTaskEditModalProps) {
  ...
  const { learnStepEmojiPreference } = useNavigationPreferenceStore();
  ...
  const [editingSubtaskEmojiId, setEditingSubtaskEmojiId] = useState<string | null>(null);
  ...
  const addSubtask = () => {
    ...
    const newSubtask = createSubTasksFromTitles([newSubtaskTitle.trim()])[0];
  ...
  const updateSubtaskEmoji = (subtaskId: string, emoji: string) => {
    ...
    setSubtasks(subtasks.map(st =>
      st.id === subtaskId ? { ...st, emoji } : st
    ));
    setEditingSubtaskEmojiId(null);
    setCustomEmojiInput('');
  };

  const resetSubtaskEmojiToSmart = (subtaskId: string) => {
    const subtaskIndex = subtasks.findIndex(st => st.id === subtaskId);
    if (subtaskIndex < 0) return;

    const recommendations = getStepEmojiRecommendations(subtasks.map(subtask => subtask.title));
    const nextEmoji = recommendations[subtaskIndex]?.[0];
    if (!nextEmoji) return;

    setSubtasks(subtasks.map(st =>
      st.id === subtaskId ? { ...st, emoji: nextEmoji } : st
    ));
    setEditingSubtaskEmojiId(null);
    setCustomEmojiInput('');
  };

  const handleCustomEmojiSubmit = (subtaskId: string) => {
    const nextEmoji = customEmojiInput.trim();
    if (!nextEmoji) return;
    updateSubtaskEmoji(subtaskId, Array.from(nextEmoji)[0]);
  };

  const emojiRecommendations = useMemo(() => {
    const recommendations = getStepEmojiRecommendations(subtasks.map(subtask => subtask.title));
    return subtasks.reduce<Record<string, string[]>>((acc, subtask, index) => {
      const currentEmoji = subtask.emoji ? [subtask.emoji] : [];
      acc[subtask.id] = Array.from(new Set([
        ...currentEmoji,
  ...
  const handleGenerateSubtasks = async () => {
    ...
    try {
      ...
      const newSubtasks = createSubTasksFromTitles(subTaskTitles);
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const nextEditingId = editingSubtaskEmojiId === subtask.id ? null : subtask.id;
                          setEditingSubtaskEmojiId(nextEditingId);
                          setCustomEmojiInput(nextEditingId === subtask.id ? (subtask.emoji || '') : '');
                        }}
                        className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg border border-dashed border-gray-300 bg-white text-base transition-all hover:border-purple-400 hover:bg-purple-50 dark:border-gray-600 dark:bg-gray-900 dark:hover:bg-purple-900/20"
                        title="只修改这个步骤的 emoji"
                      >
                        {subtask.emoji || '🙂'}
                      </button>
                    ...
                    {editingSubtaskEmojiId === subtask.id && (
                      <div className="mt-2 ml-7 rounded-lg border border-purple-200 bg-white/90 p-2 dark:border-purple-800 dark:bg-gray-900">
                        <div className="mb-1 text-[11px] font-semibold text-purple-700 dark:text-purple-300">
                          给这个步骤单独换 emoji
                        </div>
                        ...
                        <div className="mb-2 flex items-center gap-2">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                handleCustomEmojiSubmit(subtask.id);
                              }
                            }}
                            placeholder="输入任意 emoji"
                            className="flex-1 rounded-lg border border-gray-200 bg-white px-2 py-1 text-sm outline-none focus:border-purple-400 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                          />
                          <button
                            type="button"
                            onClick={() => handleCustomEmojiSubmit(subtask.id)}
                            className="rounded-lg bg-purple-100 px-2 py-1 text-xs font-semibold text-purple-700 transition-all hover:bg-purple-200 dark:bg-purple-900/30 dark:text-purple-200"
                          >
                            应用
                          </button>
                          <button
                            type="button"
                            onClick={() => resetSubtaskEmojiToSmart(subtask.id)}
                            className="flex items-center gap-1 rounded-lg bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700 transition-all hover:bg-amber-200 dark:bg-amber-900/30 dark:text-amber-200"
                            title="重置为智能推荐"
                          >
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(emojiRecommendations[subtask.id] || []).map((emojiOption) => (
                            <button
                              key={`${subtask.id}-${emojiOption}`}
                              type="button"
                              onClick={() => updateSubtaskEmoji(subtask.id, emojiOption)}
                              className={`flex h-8 w-8 items-center justify-center rounded-lg border text-base transition-all ${
                                subtask.emoji === emojiOption
                                  ? 'border-purple-500 bg-purple-100 dark:bg-purple-900/30'
                                  : 'border-gray-200 bg-gray-50 hover:border-purple-300 hover:bg-purple-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-purple-900/20'
                              }`}
                              title={`只应用到「${subtask.title}」`}
                            >
                              {emojiOption}
                            </button>
                          ))}
