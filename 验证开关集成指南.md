# 🔧 验证开关集成指南

## 📋 功能说明

为任务添加"验证流程开关"，实现以下逻辑：
- ✅ **开启验证**：点击 start/完成 按钮 → 触发验证流程（拍照 + 倒计时）
- ⚡ **关闭验证**：点击 start/完成 按钮 → 仅记录真实时间（无验证）

## ✅ 已完成的代码

### 1. 验证管理器（已实现）

**文件**：`src/hooks/useTaskVerificationManager.ts`

已实现的功能：
- ✅ `manualStartTask(taskId)` - 手动开始任务
- ✅ `manualCompleteTask(taskId)` - 手动完成任务
- ✅ 内置验证开关判断逻辑

```typescript
// 第 70-80 行：启动验证开关判断
const handleStartVerification = async (task: Task) => {
  // 🔧 开关判断
  if (!task.verificationStart) {
    console.log('⚡ 未开启验证开关，直接记录开始时间');
    await updateTask(task.id, {
      status: 'in_progress',
      actualStart: new Date(),
    });
    return;
  }
  // 开启了验证，执行验证流程...
};

// 第 160-170 行：完成验证开关判断
const handleCompleteVerification = async (task: Task) => {
  // 🔧 开关判断
  if (!task.verificationComplete) {
    console.log('⚡ 未开启验证开关，直接记录完成时间');
    await updateTask(task.id, {
      status: 'completed',
      actualEnd: new Date(),
    });
    return;
  }
  // 开启了验证，执行验证流程...
};
```

## 🎯 需要集成的位置

### 在任务卡片中添加按钮点击处理

找到渲染 **start 按钮** 和 **完成按钮** 的组件，添加以下代码：

```typescript
import { useTaskVerificationManager } from '@/hooks/useTaskVerificationManager';

function YourTaskCard({ task }: { task: Task }) {
  // 1️⃣ 引入验证管理器
  const { manualStartTask, manualCompleteTask } = useTaskVerificationManager();

  // 2️⃣ 处理开始按钮点击
  const handleStartClick = async () => {
    console.log('🎯 点击开始按钮:', task.title);
    await manualStartTask(task.id);
  };

  // 3️⃣ 处理完成按钮点击
  const handleCompleteClick = async () => {
    console.log('🎯 点击完成按钮:', task.title);
    await manualCompleteTask(task.id);
  };

  return (
    <div className="task-card">
      {/* 4️⃣ Start 按钮 */}
      {task.status === 'scheduled' && (
        <button onClick={handleStartClick}>
          start
        </button>
      )}
      
      {/* 5️⃣ 完成按钮 */}
      {task.status === 'in_progress' && (
        <button onClick={handleCompleteClick}>
          完成
        </button>
      )}
    </div>
  );
}
```

## 📂 可能需要修改的文件

根据您的截图，start 按钮可能在以下文件中：

1. **`src/components/calendar/NewTimelineView.tsx`** - 时间轴视图
2. **`src/components/calendar/TaskCard.tsx`** - 日历任务卡片
3. **`src/components/task/TaskCard.tsx`** - 通用任务卡片

### 查找方法

在这些文件中搜索：
- `"start"` 按钮文本
- `onClick` 处理函数
- 任务状态判断 `task.status`

## 🔍 验证开关的数据结构

任务对象中的验证开关字段：

```typescript
interface Task {
  id: string;
  title: string;
  status: 'scheduled' | 'in_progress' | 'completed';
  
  // 🔧 验证开关字段
  verificationStart?: {
    enabled: boolean;
    requirement?: string;
    timeout?: number;
  };
  
  verificationComplete?: {
    enabled: boolean;
    requirement?: string;
    timeout?: number;
  };
  
  // 其他字段...
}
```

### 判断逻辑

```typescript
// 判断是否开启启动验证
const hasStartVerification = !!task.verificationStart;

// 判断是否开启完成验证
const hasCompleteVerification = !!task.verificationComplete;
```

## 🎨 UI 优化建议

### 按钮样式区分

```typescript
// 开启验证：显示相机图标
{task.verificationStart ? (
  <button onClick={handleStartClick}>
    <Camera /> start
  </button>
) : (
  <button onClick={handleStartClick}>
    <Play /> start
  </button>
)}
```

### 状态提示

```typescript
{/* 显示验证开关状态 */}
{task.verificationStart && (
  <span className="badge">📸 启动验证</span>
)}
{task.verificationComplete && (
  <span className="badge">📸 完成验证</span>
)}
```

## 🚀 工作流程

### 场景 1：开启验证开关

```
用户点击 start 按钮
    ↓
调用 manualStartTask(taskId)
    ↓
检查 task.verificationStart ✅ 存在
    ↓
触发验证流程：
  1. 显示全屏验证界面
  2. 要求拍照上传
  3. 启动倒计时（2分钟）
  4. AI 验证照片
    ↓
验证通过 → 记录 actualStart + 奖励金币
验证失败 → 任务失败 + 扣除金币
```

### 场景 2：关闭验证开关

```
用户点击 start 按钮
    ↓
调用 manualStartTask(taskId)
    ↓
检查 task.verificationStart ❌ 不存在
    ↓
直接记录 actualStart
    ↓
任务状态变为 in_progress
    ↓
完成 ✅（无验证流程）
```

## 📝 测试步骤

### 测试 1：开启验证

1. 创建任务，设置 `verificationStart: { enabled: true }`
2. 点击 start 按钮
3. **预期**：显示验证界面，要求拍照 ✅

### 测试 2：关闭验证

1. 创建任务，不设置 `verificationStart`
2. 点击 start 按钮
3. **预期**：直接开始任务，无验证界面 ✅

### 测试 3：完成验证

1. 任务进行中，设置 `verificationComplete: { enabled: true }`
2. 点击完成按钮
3. **预期**：显示验证界面 ✅

### 测试 4：快速完成

1. 任务进行中，不设置 `verificationComplete`
2. 点击完成按钮
3. **预期**：直接完成任务 ✅

## 🎯 核心优势

1. **低侵入**：只需在按钮点击处调用 2 个方法
2. **零修改**：验证逻辑已在 hook 中实现，无需修改核心代码
3. **灵活控制**：每个任务可独立设置验证开关
4. **向后兼容**：不影响现有功能

## 📦 完整示例代码

```typescript
import React from 'react';
import { Camera, Play, Check } from 'lucide-react';
import { useTaskVerificationManager } from '@/hooks/useTaskVerificationManager';
import type { Task } from '@/types';

export default function TaskCard({ task }: { task: Task }) {
  const { manualStartTask, manualCompleteTask } = useTaskVerificationManager();

  return (
    <div className="task-card">
      <h3>{task.title}</h3>
      
      {/* 验证开关状态提示 */}
      <div className="badges">
        {task.verificationStart && <span>📸 启动验证</span>}
        {task.verificationComplete && <span>📸 完成验证</span>}
      </div>
      
      {/* Start 按钮 */}
      {task.status === 'scheduled' && (
        <button onClick={() => manualStartTask(task.id)}>
          {task.verificationStart ? <Camera /> : <Play />}
          start
        </button>
      )}
      
      {/* 完成按钮 */}
      {task.status === 'in_progress' && (
        <button onClick={() => manualCompleteTask(task.id)}>
          {task.verificationComplete ? <Camera /> : <Check />}
          完成
        </button>
      )}
    </div>
  );
}
```

## ✅ 下一步

1. 找到渲染 start 按钮的组件文件
2. 添加 `useTaskVerificationManager` hook
3. 将按钮的 `onClick` 连接到 `manualStartTask` / `manualCompleteTask`
4. 测试验证开关功能

**所有核心逻辑已实现，只需连接按钮即可！** 🎉

