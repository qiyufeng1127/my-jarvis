# 验证开关功能 - 使用说明

## 📋 功能概述

任务现在支持"验证流程"开关，可以灵活控制是否需要启动验证和完成验证：

- **开关开启**：点击 Start/完成 按钮 → 触发验证流程（拍照验证 + 倒计时）
- **开关关闭**：点击 Start/完成 按钮 → 仅记录真实时间，无验证流程

## 🔧 实现方式（低侵入式）

### 1. 核心修改点

#### `useTaskVerificationManager.ts` - 添加开关判断

```typescript
// 在 handleStartVerification 中添加开关判断
const handleStartVerification = async (task: Task) => {
  // 🔧 开关判断：如果未开启验证，直接记录开始时间
  if (!task.verificationStart) {
    console.log('⚡ 未开启验证开关，直接记录开始时间');
    await updateTask(task.id, {
      status: 'in_progress',
      actualStart: new Date(),
    });
    return;
  }
  
  // 开启了验证，执行验证流程...
};

// 在 handleCompleteVerification 中添加开关判断
const handleCompleteVerification = async (task: Task) => {
  // 🔧 开关判断：如果未开启验证，直接记录完成时间
  if (!task.verificationComplete) {
    console.log('⚡ 未开启验证开关，直接记录完成时间');
    await updateTask(task.id, {
      status: 'completed',
      actualEnd: new Date(),
    });
    return;
  }
  
  // 开启了验证，执行验证流程...
};

// 新增手动触发接口
export function useTaskVerificationManager() {
  // ...
  
  const manualStartTask = async (taskId: string) => {
    const task = tasks.find(t => t.id === taskId);
    if (!task) return;
    await handleStartVerification(task);
  };

  const manualCompleteTask = async (taskId: string) => {
    const task = tasks.find(t => t.id === taskId);
    if (!task) return;
    await handleCompleteVerification(task);
  };

  return {
    handleStartVerification,
    handleCompleteVerification,
    manualStartTask,      // 👈 供按钮调用
    manualCompleteTask,   // 👈 供按钮调用
    calculateBaseGold,
    calculateStartGold,
    calculateCompleteGold,
  };
}
```

#### `TaskVerificationExtension.tsx` - 适配开关

```typescript
interface TaskVerificationData {
  taskId: string;
  taskTitle: string;
  hasVerification: boolean; // 🔧 新增：是否开启验证开关
  // ...
}

// 在监听事件时判断开关
useEffect(() => {
  const handleTaskTimeArrived = (data: TaskVerificationData) => {
    // 🔧 开关判断：如果未开启验证，不显示验证界面
    if (!data.hasVerification) {
      console.log('⏭️ 未开启验证开关，跳过验证界面');
      return;
    }
    
    // 开启了验证，显示验证界面...
  };
  
  eventBus.on('taskTimeArrived', handleTaskTimeArrived);
  // ...
}, []);
```

### 2. 在任务卡片中使用

```typescript
import { useTaskVerificationManager } from '@/hooks/useTaskVerificationManager';

function TaskCard({ task }: { task: Task }) {
  const { manualStartTask, manualCompleteTask } = useTaskVerificationManager();
  
  return (
    <div className="task-card">
      <h3>{task.title}</h3>
      
      {/* Start 按钮 */}
      {task.status === 'scheduled' && (
        <button onClick={() => manualStartTask(task.id)}>
          {task.verificationStart ? '启动验证' : '开始任务'}
        </button>
      )}
      
      {/* 完成按钮 */}
      {task.status === 'in_progress' && (
        <button onClick={() => manualCompleteTask(task.id)}>
          {task.verificationComplete ? '完成验证' : '完成任务'}
        </button>
      )}
    </div>
  );
}
```

## 🎯 工作流程

### 开启验证开关时

```
用户点击 Start 按钮
    ↓
manualStartTask(taskId)
    ↓
handleStartVerification(task)
    ↓
检查 task.verificationStart ✅ 存在
    ↓
更新状态为 'verifying_start'
    ↓
显示验证界面（拍照 + 倒计时）
    ↓
用户完成验证
    ↓
更新状态为 'in_progress' + 记录 actualStart
```

### 关闭验证开关时

```
用户点击 Start 按钮
    ↓
manualStartTask(taskId)
    ↓
handleStartVerification(task)
    ↓
检查 task.verificationStart ❌ 不存在
    ↓
直接更新状态为 'in_progress' + 记录 actualStart
    ↓
完成（无验证流程）
```

## 📊 开关状态判断

```typescript
// 判断是否开启启动验证
const hasStartVerification = !!task.verificationStart;

// 判断是否开启完成验证
const hasCompleteVerification = !!task.verificationComplete;

// 完全关闭验证
const noVerification = !task.verificationStart && !task.verificationComplete;
```

## ✅ 优势

1. **低侵入**：只修改了 3 个函数，新增了 2 个接口
2. **向后兼容**：不影响现有代码，完全兼容
3. **灵活控制**：每个任务可独立设置是否需要验证
4. **逻辑清晰**：开关判断在最前面，一目了然
5. **易于维护**：代码结构清晰，注释完整

## 🔍 调试日志

```
// 开启验证时
🚀 [验证管理器] 触发启动验证: 任务名称
🔔 收到任务验证触发事件: {...}
✅ [验证管理器] 启动验证成功: 任务名称

// 关闭验证时
🚀 [验证管理器] 触发启动验证: 任务名称
⚡ [验证管理器] 未开启验证开关，直接记录开始时间
```

## 📝 注意事项

1. **类型定义**：`Task` 类型中的 `verificationStart` 和 `verificationComplete` 字段是可选的
2. **状态流转**：关闭验证时，状态直接从 `scheduled` → `in_progress` → `completed`
3. **金币计算**：关闭验证时，可以根据实际时长计算金币（无验证奖励/惩罚）
4. **时间记录**：无论是否开启验证，都会记录 `actualStart` 和 `actualEnd`

## 🚀 下一步扩展

如果需要更细粒度的控制，可以添加：

1. **仅启动验证**：`verificationStart` 存在，`verificationComplete` 不存在
2. **仅完成验证**：`verificationStart` 不存在，`verificationComplete` 存在
3. **验证严格度**：`verificationStrictness: 'low' | 'medium' | 'high'`
4. **验证类型**：`verificationType: 'photo' | 'text' | 'location'`





